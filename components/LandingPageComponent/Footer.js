...
export default function Footer() {
  ...
  return (
    <footer id="subscribe" className="w-11/12 m-auto rounded-t-2xl p-10 pt-20 font-poppins text-gray-200 shadow-2xl">
      <div ref={ref} className={`flex flex-col items-center justify-center ${inView && "slide-in"}`}>
        <h1 className="text-3xl font-righteous">Get notified when we launch</h1>
        <p className="text-lg pt-2">Stay up to date with the latest news,annoucements, and articles.</p>
        <div className="mt-8 flex border border-gray-50 pl-0 rounded-lg">
          <input
            ...
            "
          />
          <ButtonCst className="ml-4 py-2 bg-cstColor-200 text-white shadow-sm scale-95 m-1 mr-2" onClick={subcribeHandler}>
            Subscribe
          </ButtonCst>
        </div>
      </div>
      ...
      <div className={`p-8 flex justify-between text-gray-200 font-montserrat  ${inView && "slide-in"}`}>
        <div className="w-1/2">
          <span className="flex items-center text-2xl">
            <BsListTask className="mr-2" />
            <h2 className="font-righteous font-bold">TaskTribe</h2>
          </span>
          <p className="mt-4 w-3/5">Stay organized. keep track of your to-do list, deadlines, and progress on projects.</p>
          <div className="mt-8 w-4/5 flex justify-between font-sans font-bold">
            ...
            <Link href="#features" className="cursor-pointer hover:text-cstColor-200">
              Features
            </Link>
            <Link href="#reviews" className="cursor-pointer hover:text-cstColor-200">
              Reviews
            </Link>
            <Link href="#faq" className="cursor-pointer hover:text-cstColor-200">
              Faq&aposs
            </Link>
            <Link href="#footer" className="cursor-pointer hover:text-cstColor-200">
              Contact
            </Link>
          </div>
        </div>
        <div className="flex flex-col">
          ...
          <LinkCst href="/authentication/signup" className="mt-2 bg-gray-100 text-black shadow-sm">
            Get Started
          </LinkCst>
          <LinkCst href="/authentication" className="mt-4 bg-cstColor-200 text-white shadow-sm">
            Continue
          </LinkCst>
        </div>
      </div>
      ...
      <div className={`mt-8 flex justify-between font-montserrat ${inView && "slide-in"}`}>
