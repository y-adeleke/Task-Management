...
export default function Faq() {
  ...
  const showMoreHandler = () => setShowMore(FaqData.length - showMore > 4 ? showMore + 4 : FaqData.length);
  const moreQuestionHandler = () => setMoreQuestionForm(true);
  const sendMoreQuestionHandler = () => {
    //logic here
  ...
  return (
    <section id="faq" ref={ref} className={`h-auto p-10 pt-20 px-20 font-poppins ${inView && "slide-in"}`}>
      <h1 className="text-start text-5xl font-righteous w-5/10 mx-auto pb-1 uppercase text-cstColor-100">You have questions. We have answers.</h1>
      <p className="text-lg pt-3">Quick answers to questions you may have. Have more questions? Kindly, reach out to us.</p>
          ...
          if (key + 1 <= showMore) {
            return (
              <div className="flex slide-in" key={key}>
                ...
                <div className="pl-2">
                  <h3 className="font-bold pb-1 font-poppins">{content.question}</h3>
                  <p className="pl-2 ml-4 text-gray-800 font-montserrat border-2 border-transparent border-l-cstColor-100">{content.answer}</p>
                </div>
              </div>
            );
          }
      ...
      {showMore != FaqData.length && (
        <div className="w-max m-auto mt-8 hover:tranform hover:scale-110 transition duration-200">
          <ButtonCst className="border-2 py-2 border-gray-200  text-black shadow-sm" onClick={showMoreHandler}>
            Show more
          </ButtonCst>
        </div>
      )}

      <div className={`mt-8 w-3/5 slide-in border-2 rounded-2xl px-4 py-4 m-auto ${moreQuestionForm && "bg-cstColor-600 text-white"}`}>
        {!moreQuestionForm && (
          <div className="flex justify-between slide-in">
            <div className="flex">
              <Image src={profileImg} alt="profile image" className="w-12 h-12 rounded-full mr-2" width={400} height={400} />
              <div>
                <h3>More Questions?</h3>
                <p>You still have more questions? Kidnly reach out.</p>
              </div>
            </div>
            <ButtonCst className="border-2 py-2 bg-black text-gray-50 " onClick={moreQuestionHandler}>
              Get in touch
            </ButtonCst>{" "}
          </div>
        )}
        {moreQuestionForm && (
          <div className="w-full px-4 slide-in">
            ...
            "
              />
              <ButtonCst className="ml-4 py-2 bg-cstColor-200 text-white shadow-sm scale-95 m-1 mr-2" onClick={sendMoreQuestionHandler}>
                Send
              </ButtonCst>
            </div>
          </div>
